...
pub trait Phidget {
    ...
    fn open(&mut self) -> Result<()> {
        unsafe { crate::check_ret(ffi::Phidget_open(self.as_handle())) }
    }

    fn open_wait(&mut self, to: Duration) -> Result<()> {
        let ms = to.as_millis() as u32;
        unsafe { crate::check_ret(ffi::Phidget_openWaitForAttachment(self.as_handle(), ms)) }
    }

    /// Closes the channel
    fn close(&mut self) -> Result<()> {
        unsafe { crate::check_ret(ffi::Phidget_close(self.as_handle())) }
    }
    ...
    fn set_local(&mut self, local: bool) -> Result<()> {
        ...
        unsafe { crate::check_ret(ffi::Phidget_setIsLocal(self.as_handle(), local)) }
    }
    ...
    fn set_remote(&mut self, rem: bool) -> Result<()> {
        ...
        unsafe { crate::check_ret(ffi::Phidget_setIsRemote(self.as_handle(), rem)) }
    }
}
